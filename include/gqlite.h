...
#define GQLITE_HAS_CONNECTION_CREATE_FROM_FILE
...
namespace gqlite
{
  ...
  public:
    ...
    value(
      const std::initializer_list<typename std::unordered_map<std::string, _T_>::value_type>& _v);
  ...
  public:
     ...
     */
    [[deprecated(
      "Not possible to create a database from a sqlite pointer anymore.")]] static connection
      create_from_sqlite(void* _pointer, const value& _options = value());
    /**
     * Create a sqlite connection from a file.
     */
    [[deprecated(
      "Use `create_from_file` and select sqlite as a backend in options.")]] static connection
      create_from_sqlite_file(const std::string& _filename, const value& _options = value());
    /**
     * Create a connection from a file.
  ...
  public:
     ...
     * Execute a query.
     */
    value execute_oc_query(const std::string& _string, const value_map& _bindings = {});
  private:
    struct data;
    std::shared_ptr<data> d;
  };
