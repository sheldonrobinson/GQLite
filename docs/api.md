...
* `gqlite.Connection` is the main class for connection to the database.
  * `gqlite.Connection(filename)` is used to create a new connection to a gqlite database stored in a persy database
...
* `gqlite::Connection` is the main class for connection to the database.
  * `gqlite::Connection.new(filename)` is used to create a new connection to a gqlite database stored in a persy database
  * `execute_oc_query(query, parameters)` is used to query the database, `parameters` is an optional map.
