...
{
  template<typename T>
  concept std_to_stringable = requires(T a) { std::to_string(a); };
  template<typename T>
  concept container_begin_end = requires(T a) {
    a.begin();
    a.end();
  };
  ...
  template<typename _T_>
    requires std_to_stringable<_T_>
  struct to_string_impl<_T_>
  {
    static inline std::string to_string(const _T_& _v) { return std::to_string(_v); }
  };

  template<typename _T_>
    requires container_begin_end<_T_>
  struct to_string_impl<_T_>
  {
  ...
  namespace details
  {
    inline std::string format_string(std::size_t, const std::string& _format) { return _format; }
    template<typename _T_, typename... _Targs_>
    inline std::string format_string(std::size_t _start_pos, const std::string& _format,
                                     const _T_& _a, const _Targs_&... _args)
    {
