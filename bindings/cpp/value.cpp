...
struct value::data
{
  ...
  value_type type;
  std::variant<invalid, bool, int64_t, double, std::string, value_map, value_vector>
    value_container;
  ...
  struct json_reader
  {
    ...
    void fetch_next_non_space_char()
    {
      while(std::isspace(fetch_next_char()))
      {
        ...
        {
          throw_exception("Invalid json in stream.");
        }
      }
    };
    int fetch_next_char() { return (last_char = stream.get()); }
    /// @brief start parsing
    ...
    std::string read_string()
    {
      ...
      fetch_next_char();
      std::string string;
      bool keep_nc = false; // this is used to indicate if the last character was
                            // a '\' or not
      while(keep_nc or last_char != '"')
      {
        if(not keep_nc and last_char == '\\')
        {
          keep_nc = true; // make sure to ignore the next character if it is a '"'
                          // or to properly handle a '\n'
        }
        else
        {
          if(keep_nc)
        ...
        throw_exception_if(not stream.good(), "Unfinished string: {}.", string);
        fetch_next_char();
      }
    ...
    void is_char(int _c)
    {
      throw_exception_if(last_char != _c, "Expected '{}' but got '{}'.", char(_c), char(last_char));
    }
    ...
    gqlite::value read_value()
    {
      ...
      case '9':
      {
        ...
        while((last_char >= '0' and last_char <= '9') or last_char == '.' or last_char == 'e'
      ...
      default:
        throw_exception("Unexpected '{}'.", char(last_char));
      }
    }
  };
} // namespace
...
value_type value::get_type() const { return d->type; }
...
bool value::operator<(const value& _rhs) const
{
  throw_exception_if(d->type != _rhs.d->type,
                     "Value of different types are not comparable with '<'");
  switch(d->type)
  {
  case value_type::boolean:
  ...
  default:
    throw_exception("Cannot compare {} and {} with inferior operator.", *this, _rhs);
  }
}
...
bool value::to_bool() const
{
  ...
  default:
    throw_exception("Value is not a bool, it is {}.", d->type);
  }
}
...
int64_t value::to_integer() const
{
  ...
  default:
    throw_exception("Value is not a number, it is '{}'.", d->type);
  }
}
...
{
  throw_exception_if(d->type != value_type::string, "Value is not a string, it is '{}'.", d->type);
  return std::get<std::string>(d->value_container);
}

value_map value::to_map() const
{
  throw_exception_if(d->type != value_type::map, "Value is not a map, it is '{}'.", d->type);
  return std::get<value_map>(d->value_container);
}

value_vector value::to_vector() const
{
  throw_exception_if(d->type != value_type::vector, "Value is not a vector, it is '{}'.", d->type);
